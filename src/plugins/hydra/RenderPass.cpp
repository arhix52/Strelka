...
void HdStrelkaRenderPass::_BakeMeshes(HdRenderIndex* renderIndex, GfMatrix4d rootTransform)
{
    ...
    auto getOrCreateMaterial = [&](const SdfPath& materialId) {
        ...
        else
        {
            ...
            materialIndex = mScene->addMaterial(material->getDescription());
    ...
    for (const auto& rprimId : renderIndex->GetRprimIds())
    {
        ...
        if (dynamic_cast<const HdMesh*>(rprim))
        {
            ...
            if (materialId.IsEmpty())
            {
                ...
                material.params.material_type = MATERIAL_TYPE_STANDARD_PBR;
                material.params.base_color = {color[0], color[1], color[2]};
                material.params.metallic = 0.0f;
                material.params.roughness = 0.5f;
                material.params.ior = 1.5f;
                material.params.base_color_tex = -1;
                material.params.metallic_roughness_tex = -1;
                material.params.normal_tex = -1;
                material.params.emission_tex = -1;
                material.params.occlusion_tex = -1;
                material.params.transmission_tex = -1;

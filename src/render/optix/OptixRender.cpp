...
Texture OptiXRender::loadTextureFromFile(const std::string& fileName)
{
    ...
    // Track resources for cleanup, in the material set: these are released and
    // reloaded whenever createOptixMaterials() runs again.
    mMaterialTextureArrays.push_back(device_tex_array);
    mMaterialTextureObjects.push_back(tex_obj);
    mMaterialTextureObjects.push_back(tex_obj_unfilt);

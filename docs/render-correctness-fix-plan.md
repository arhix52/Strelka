...
execution on an actual `MTLDevice` are reported separately. OptiX execution remains external where stated.
...
## Finding AP: SHARC numeric undefined behavior

- Random variables, measure, PDFs, PMFs, delta classification, and MIS: none. This is deterministic cache-grid
  addressing and temporal reprojection, outside the rendering estimator's probability measures.
- Support: a finite coordinate in the signed 32-bit range retains its exact floored cell. NaN maps to the neutral
  coordinate zero; infinities and finite overflow saturate by sign. Reprojection compares the true squared integer
  distances for every coordinate representable by the packed 17-bit voxel key.
- Reproducer: UBSan reported NaN-to-`int32_t` conversion in `voxelCoordinate()` and signed multiplication/addition
  overflow in both distance expressions in `adjacentLevelKey()`. At camera coordinates 100,000 and 200,000, the old
  wrapped 32-bit sums could choose the wrong adjacent grid level.
- Implementation and result: guard the float conversion before its cast, widen coordinate differences to `int64_t`,
  and sum their individually safe squares as `uint64_t`. Signed left shifts were replaced with safe multiplication.
  The focused regression passes 17/17 assertions in Debug, Release, and ASan+UBSan. The full sanitizer suite passes
  929/929 cases and 69,256,863/69,256,863 assertions with no `sharc_grid.h` diagnostic.
